...
pub fn append_setup_script(name: impl AsRef<Path>, io: impl FnOnce(&mut std::fs::File) -> io::Result<()>) {
    let name = name.as_ref();
    std::fs::create_dir_all(".container/scripts/setup").unwrap_or_else(|err| fatal!("unable to create .container/scripts/setup/: {}", err));
    let path = Path::new(".container/scripts/setup").join(name);
    let exists = path.exists();
    let mut o = std::fs::OpenOptions::new().create(true).write(true).append(true).open(&path).unwrap_or_else(|err| fatal!("unable to open .container/scripts/setup/{}: {}", name.display(), err));
    if !exists {
        (|o: &mut std::fs::File| -> io::Result<()> {
            writeln!(o, "#!/usr/bin/sh")?;
            writeln!(o)?;
            writeln!(o, "# AUTOGENERATED BY {}", env!("CARGO_PKG_NAME"))?;
            writeln!(o, "for_apt_get() {{")?;
            writeln!(o, "    if [ \"$APT_GET_MODE\" = \"\" ]; then")?;
            writeln!(o, "        if apt-get --version >/dev/null 2>/dev/null; then")?;
            writeln!(o, "            echo $ apt-get update")?;
            writeln!(o, "            apt-get update || exit 1")?;
            writeln!(o, "            APT_GET_MODE=use")?;
            writeln!(o, "        else")?;
            writeln!(o, "            APT_GET_MODE=skip")?;
            writeln!(o, "        fi")?;
            writeln!(o, "    fi")?;
            writeln!(o, "    if [ \"$APT_GET_MODE\" != \"skip\" ]; then")?;
            writeln!(o, "        echo $ apt-get $@")?;
            writeln!(o, "        apt-get $@ || exit 1")?;
            writeln!(o, "    fi")?;
            writeln!(o, "}}")?;
            Ok(())
        })(&mut o).unwrap_or_else(|err| fatal!("error appending to .container/scripts/setup/{}: {}", name.display(), err));
    }
    io(&mut o).unwrap_or_else(|err| fatal!("error appending to .container/scripts/setup/{}: {}", name.display(), err));
    if !exists {
        #[cfg(unix)] {
            use std::os::unix::fs::PermissionsExt;
            let mut perms = path.metadata().unwrap_or_else(|err| fatal!("failed to get permissions for {}: {}", path.display(), err)).permissions();
            perms.set_mode(0o755);
            std::fs::set_permissions(&path, perms).unwrap_or_else(|err| fatal!("failed to set permissions for {}: {}", path.display(), err));
        }
    }
}
